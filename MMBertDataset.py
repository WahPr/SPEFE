...
class MMBertDataset(Dataset):
    ...
    def create_text_sentence(self, i):
        ...
        To be..
        """
        firstSentence = self.items[i][0][0]   #mosi：长度为40
        sentiment = self.sentiment_selection(self.items[i][1][0], str(self.num_labels))
        label = 0
        
        firstTokenTypeIds = np.zeros(len(firstSentence))  #mosi：创建一个长度为40的0矩阵
    ...
    def __len__(self):
        ...
        """
        return self.total_item   #返回划分的数据集，变成列表(mosi为1283长度的列表)/
