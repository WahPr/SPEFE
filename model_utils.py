...
def mask_tokens(inputs, tokenizer, args):     #[文本传入padded_text_ids，视觉传入tWv_examples，语音传入tWs_examples]、分词器、参数
    """
...
def pad_example(examples, padding_value=0):
    #padding_value will be tokenizer.pad_token_id  将句子填充到相同的长度
    """
...
def collate(examples):
    ...
    rawData = [None] * len(examples)        #创建列表，其长度与example列表相同，初始值为None
