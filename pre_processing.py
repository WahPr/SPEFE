...
def download_dataset(datasetName):
    ...
    try:
        ...
        print("Labels have been downloaded previously.")  # 载入高级特征、原始数据和标签

    TRAINSPLIT = DATASET.standard_folds.standard_train_fold
    VALSPLIT = DATASET.standard_folds.standard_valid_fold
    TESTSPLIT = DATASET.standard_folds.standard_test_fold  # 分割数据集为train、test、val
...
def prepare_save(features, dataset, TRAINSPLIT, VALSPLIT, TESTSPLIT):
    ...
    # 返回数据集的形式：每个数记录内容为（words，visual，speech）， label，segment
...
def main():
    ...
    parser.add_argument("--textField", type=str, default='CMU_MOSI_ModifiedTimestampedWords')  #原始的
    # parser.add_argument("--textField", type=str, default='CMU_MOSI_TimestampedWords')
    # parser.add_argument("--textField", type=str, default='CMU_MOSEI_TimestampedWords')
    parser.add_argument("--visualField", type=str, default='CMU_MOSI_Visual_Facet_41')
    # parser.add_argument("--visualField", type=str, default='CMU_MOSEI_VisualFacet42')
    parser.add_argument("--speechField", type=str, default='CMU_MOSI_COVAREP')
    # parser.add_argument("--speechField", type=str, default='CMU_MOSEI_COVAREP')
    parser.add_argument("--labelField", type=str, default='CMU_MOSI_Opinion_Labels')
    # parser.add_argument("--labelField", type=str, default='CMU_MOSEI_Labels')
    # parser.add_argument("--datasetName", type=str, required=True) #原始的
    parser.add_argument("--datasetName", type=str, default='cmu_mosi')
    # parser.add_argument("--datasetName", type=str, default='cmu_mosei')
    ...
    recipe = {feat: os.path.join(DATA_PATH, feat) + '.csd' for feat in features}  # 构建数据集
