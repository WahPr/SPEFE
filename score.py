...
def MISA(test_truth,test_preds):
    ...
    if True:
        ...
        print("cm:", confusion_matrix(binary_truth, binary_preds))
...
def main():
    ...
    # preds = np.load(os.path.join('numpy_save', args.path, 'predict.npy'))#原始
    # labels = np.load(os.path.join('numpy_save', args.path, 'target.npy'))
    preds = np.load(os.path.join('/home/user1/tangyu/experiment/numpy_save/20250423-00', 'predict.npy'))
    labels = np.load(os.path.join('/home/user1/tangyu/experiment/numpy_save/20250423-00', 'target.npy'))
